...
- For "Max base fee" and "Priority fee", check the current gas prices on the [NOVAI explorer](https://scan.novaichain.com/novaichain). In this example, the base fee is 0.2 Gwei, and the priority fee is 0 Gwei.
...
![Step 3](./metamask-step3.png)

:::tip
Gas fees change based on the NOVAI network utilization. If you consistently face issues with MetaMask, you might want to consider switching to other wallets such as Rainbow or Rabby.
:::

## I can't connect to the NOVAI network. How can I change the RPC node?

If you are unable to connect to the NOVAI network, you can try changing the RPC node in your wallet. Here is an example how to change the RPC node in MetaMask:

1. Open the wallet settings.

![Settings](./metamask-settings.png)

2. Go to the "Networks" tab and select the NOVAI Network to change RPC nodes.

We recommend using the following network settings:

- **Network Name**: NOVAI Mainnet
- **New RPC URL**: https://rpc.novaichain.com
- **Chain ID**: 7256
- **Symbol**: NOVAI
- **Block Explorer URL**: https://scan.novaichain.com/
